...
public class CharacterController2 : MonoBehaviour
{
  ...
  private Rigidbody rb;
    ...
    {
        // MOUVEMENT
        moveHorizontal = Input.GetAxis ("J2_MainHorizontal");
        moveVertical = Input.GetAxis ("J2_MainVertical");
        ...
        //
        // ROTATION
        Vector3 moveDirection = new Vector3(Input.GetAxis("J2_SecHorizontal"), 0, Input.GetAxis("J2_SecVertical"));
        ...
        //
        // INPUT LEAVE BALLOON
        if (Input.GetButtonDown("J2_A_Button")){
            canShoot = true;
            canStrafe = true;
            isFollowing =false;
        }
        ...
        if (Input.GetButtonDown("J2_B_Button") && canStrafe == true)
        {
          slideDuration = 0f;
        }
        //
        if (Input.GetButtonDown("J2_Y_Button"))
        {
          isDead = true;
        }
        //
        if (slideDuration >= 0)
        {
          Vector3 moveSlideDirection = new Vector3(Input.GetAxis("J2_MainHorizontal"), 0, Input.GetAxis("J2_MainVertical"));
        ...
        slideDuration--;
        //
         if (Input.GetAxis("J2_RTrigger") > 0.5 && canShoot == true && isDead == false)
        {
          charge = true;
          movespeed = 1f;
          canShoot = true;
        ...
        else if(Input.GetAxis("J2_RTrigger") < 0.5 && charge == true )
        {
          movespeed = 4f;
          charge = false;
            Fire();
        ...
        //
        if(isDead == true && Input.GetButtonDown("J2_A_Button") || RespawnCooldown < 0)
        {
          Respawn();
        }
        //
    ...
    private void OnTriggerEnter(Collider other)
    {
      ...
      if (other.gameObject.name != "C2Bullet" && other.gameObject.tag == "Bullet")
      {
        life--;
